!> Module providing a "doubly linked" list containing
!> a single REAL64 data item per node
!> More data can be added by expanding the 'node' type
!> and any methods which set fields

!> The list and its nodes are described by a "TYPE"
!> which is like a struct, or a class with no functions
!> It "joins" multiple items into a thing you can access
!> under a single name, pass to functions as a single object etc
MODULE doubly_linked_list
  ...
  SUBROUTINE add_at_end(item, list)
  ...
  SUBROUTINE remove_item(item, list)
    ...
    TYPE(dl_list) :: list
     ...
     IF(ASSOCIATED(item, TARGET=list%head)) THEN
     ...
     ELSE IF(ASSOCIATED(item, TARGET=list%tail)) THEN
...
PROGRAM MAIN
  ...
  DO i = 1, n_part
    ...
    CALL add_at_end(current, electron)
  ...
  DO WHILE(ASSOCIATED(current%next))
    ...
    IF (current%gamma - 1.0_REAL64 > inc) THEN
      ! Move to fast list
      CALL remove_item(current, electron)
      CALL add_at_end(current, fast_electron)
    END IF
